...
const mappingPria = {
    A: {
        ...
        "Carpenter/ Pengrajin Kayu": "Practical",
        Dokter: "Medical",
    },
    B: {
...
function assignRanking(pekerjaanList) {
    ...
    const pekerjaanWithPriority = Object.entries(pekerjaanList).map(
        ([nama, kategori]) => ({
            nama,
            kategori,
    ...
    Object.keys(mapping).forEach((kelompok) => {
        ...
        Object.entries(rankingMap).forEach(([namaPekerjaan, ranking]) => {
            ...
            const input = document.querySelector(
                `input[name="jawaban[${kelompok}][${namaPekerjaan}]"]`,
            );
            ...
            if (input) {
                ...
                totalFilled++;
                console.log(
                    `   ✓ ${namaPekerjaan} (${pekerjaanDiKelompok[namaPekerjaan]}) → Ranking ${ranking}`,
                );
            } else {
