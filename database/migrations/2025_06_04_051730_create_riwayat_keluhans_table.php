<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
// database/migrations/xxxx_xx_xx_create_riwayat_keluhans_table.php
    public function up(): void
    {
        Schema::create('riwayat_keluhans', function (Blueprint $table) {
            $table->id();
            $table->bigInteger('nim'); // disesuaikan jadi angka juga

            // foreign key ke data_diris.nim
            $table->foreign('nim')
                ->references('nim')
                ->on('data_diris')
                ->onDelete('cascade');

            $table->text('keluhan');
            $table->string('lama_keluhan');
            $table->enum('pernah_konsul', ['Ya', 'Tidak']);
            $table->enum('pernah_tes', ['Ya', 'Tidak']);
            $table->timestamps();
        });
    }


    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('riwayat_keluhans');
    }
};
